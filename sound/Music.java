...
public class Music {
    ...
    /**
     * Construct Music using specified instrument and a single channel.
     *
     ...
     * @param instr the integer code for the midi instrument to use for all
     *              channels
     */
    ...
    /**
     * Construct music with specified number of channels and specified instrument
     * for each channel.
     *
     * @param numCh the number of channels
    ...
    /**
     * Create and initialize the default synthesizer. Uses a default number of
     * channels and instrument.
     */
    ...
    /**
     * Create and initialize the default synthesizer. Uses default instrument.
     *
     ...
     *
     * @param numCh the number of channels to initialize
     * @param instr the integer code for the midi instrument used for every
     *              channel
     */
    ...
    /**
     * Play a chord. If the pitch for a given channel's note is identical to that
     * in the previous chord played, the previous note is not turned off and the
     * new note is not turned on.
     *
        ...
        while (chordIt.hasNext()) {
            ...
            if (note.equals(Note.NULL_NOTE)) {
                ...
                channels[note.channel()]
                    .noteOff(prev.pitch(), prev.velocityOn());
            } else if (note.pitch() != prev.pitch()) {
                // System.out.println("  Note: " + note);
                channels[note.channel()]
                    .noteOff(prev.pitch(), prev.velocityOn());
                channels[note.channel()]
                    .noteOn(note.pitch(), note.velocityOn());
                duration =
                    (duration > note.duration()) ? note.duration() : duration;
