...
/**
 * ImgProvider is responsible for managing a single image (loading, filtration,
 * rendering, etc.).
 *
...
public class ImgProvider extends JComponent {
  ...
  /**
   * Retrieve this ImgProvider's unique id. For ImageLab's internal use.
   */
  public int getid() {
    return id;
  }//getid
  ...
  /**
   * Create a B&W image object based on the parameter. Uses the instance
   * variable pix as destination.
   *
  ...
  public void readinImage() {
    ...
    grab =
        new PixelGrabber(img, xinc, yinc, width, height, true); //forceRGB=true
    try {
      grab.grabPixels();
    } catch (Exception e) {
  ...
  /**
   * Retrieve the image's raw image. Note that img is not always consistent with
   * RGBA arrays.
   *
  ...
  void save() {
    ...
    if (returnVal != JFileChooser.APPROVE_OPTION) {
      JOptionPane.showMessageDialog(myframe,
          "Encountered a problem in ImgProvider.save()" +
              "\n- Please try again.");
      return;
    }
    ...
    BufferedImage bufim =
        new BufferedImage(pixwidth, pixheight, BufferedImage.TYPE_INT_RGB);
  ...
  /**
   * Used by ImageLab to render an image as sound. Three channels of sound are
   * created, using the average the Red, Green and Blue values of each row to
   * establish the pitches for the channels. The average of Hue, Saturation and
   * Brightness values of each row are used as the velocities of the Red, Green
   * and Blue notes respectively.
   */
  public void play() {
    ...
    for (int row = 0; row < height; row++) {
      ...
      chord = new Chord();
      chord.addNote(
          new Note(0, (scale.getPitch(pitchRange * redSum / width / 256)),
              tempo, velocity[0]));
      chord.addNote(
          new Note(1, (scale.getPitch(pitchRange * greenSum / width / 256)),
              tempo, velocity[1]));
      chord.addNote(
          new Note(2, (scale.getPitch(pitchRange * blueSum / width / 256)),
              tempo, velocity[2]));
      tune.addChord(chord);
      rowSum = 0;
      redSum = 0;
